...
{
    public partial class asces1 : Form
    {
        ...
        public void rempli()
        {
            label7.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 4).ToString();
            label8.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 5).ToString();
            label9.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 6).ToString();
            label10.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 1).ToString();
            label11.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 2).ToString();
            label12.Text = db.materiel.Where(w => w.bureau.id_service == 3).Count(c => c.id_type_materiel == 3).ToString();
            dataGridView1.DataSource = db.materiel.Where(w => w.bureau.type_service.id_service ==3).Select(s => new { s.id_bureau, s.id_materiel, s.type_materiel.libelle, s.detail_materiel.etat, s.detail_materiel.fiche_technique }).ToList();
            comboBureau.DataSource = db.bureau.Where(w => w.id_service==3).Select(s => s.id_bureau).ToList();
        ...
        private void Form4_Load(object sender, EventArgs e)
        {
            try
            {
                label17.Text = db.type_service.Where(w => w.id_service == 3).Select(s => s.nom_type).Single();
